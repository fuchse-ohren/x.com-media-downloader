// ==UserScript==
// @name        x.com Media Downloader
// @name:ja     x.com Media Downloader
// @description    Save Video/Photo by One-Click.
// @description:ja ワンクリックで動画・画像を保存。
// @version     2.1
// @author      fuchse-ohren
// @namespace   none
...
// @grant       GM_download
// @license     MIT
// @downloadURL https://github.com/fuchse-ohren/x.com-media-downloader/raw/main/x_media_downloader.user.js
// @updateURL https://github.com/fuchse-ohren/x.com-media-downloader/raw/main/x_media_downloader.user.js
// ==/UserScript==

/*
 * Modified by fuchse-ohren in 2025
 * Based on original work:
 *   Copyright (c) 2025 AMANE
 *   Released under the MIT License
 */

const filename = 'x_{user-name}(@{user-id})_{date-time}_{status-id}_{file-type}';
  ...
  return {
    ...
    addButtonTo: function (article) {
      ...
      if (media) {
        ...
        if (is_tweetdeck) {
          btn_down.firstElementChild.innerHTML = '<svg viewBox="0 0 24 24" style="width: 32px; height: 32px;">' + this.svg + '</svg>';
      ...
      if (imgs.length > 1) {
        ...
        imgs.forEach(img => {
          ...
          btn_down.innerHTML = '<div><div><svg viewBox="0 0 24 24" style="width: 32px; height: 32px;">' + this.svg + '</svg></div></div>';
